...
def create_arabic_pdf(text, filename="output.pdf"):
    ...
    try:
        ...
        # حاول إضافة خط Amiri
        try:
            # تحقق إذا كان الخط موجوداً
            font_path = "fonts/Amiri-Regular.ttf"
            if not Path(font_path).exists():
                # إذا لم يكن موجوداً، استخدم خط افتراضي
                st.warning("⚠️ خط Amiri غير متوفر، سيتم استخدام خط افتراضي")
                pdf.add_font("Arabic", "", "DejaVuSans.ttf")
        ...
        # معالجة النص العربي
        # الخطوة 1: إعادة تشكيل النص
        reshaped_text = arabic_reshaper.reshape(text)
        
        # الخطوة 2: ترتيب النص من اليمين لليسار
        rtl_text = get_display(reshaped_text)
        
        # إضافة العنوان
        pdf.set_fill_color(44, 62, 80)  # لون داكن
        pdf.cell(0, 10, "نتيجة التحويل", 0, 1, 'R', fill=True)
        pdf.ln(10)
        
        # إضافة النص مع دعم الأسطر الطويلة
        pdf.set_text_color(0, 0, 0)
        pdf.multi_cell(0, 10, rtl_text)
        ...
        pdf.cell(0, 10, f"تم الإنشاء: {current_date}", 0, 1, 'R')
